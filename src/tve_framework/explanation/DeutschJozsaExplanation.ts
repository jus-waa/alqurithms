...
export function explainDeutschJozsaStep( step: number ): ExplanationResult | null {
  ...
  const measExplanations: Record<number, string> = {
    [meas_start]:"Measurement begins. q[0] is measured first, collapsing the first qubit.",
    [meas_start + 1]: "q[1] is measured, collapsing the second qubit. The combined result of q[0] and q[1] continues to reveal the balanced nature of the function.",
    [meas_end]: "q[2] is measured, all three input qubits are now measured. The result of the function is balanced.",
  };

  const explanations: Record<string, string> = {
    default: "Initially, states are |0⟩|0⟩|0⟩|0⟩. Barrier gates will be applied throughout the algorithm to separate pieces of the circuit properly.",
    prep: "A Pauli-X gate is applied on the last qubit (q[3]) to flip the ancilla from |0⟩ to |1⟩ before creating the |−⟩ state. This prepares the auxiliary qubit so that the oracle can implement f(x) into the phase.",
    superpos: "Hadamard gates are applied to all qubits, creating a superposition of all 8 possible 3-bit input strings from |000⟩ to |111⟩. This allows the algorithm to evaluate all possible inputs simultaneously enabling quantum parallelism. Each of the states will now have equal value.",
    oracle_1: "The oracle Uf is being applied to conditionally flips the auxiliary qubit for each input that evaluates to. Pauli-X gates are applied on q[0] and q[1] to select this input, followed by a Toffoli gate that conditionally flips the auxiliary qubit since f(001) = 1, then Pauli-X gates on the same qubits to restore them. The oracle conditionally flips the auxiliary qubit for each input where f(x) = 1.",
    ...
    finalh: "The final Hadamard gates are applied to the first n qubits (q[0] to q[2]), transforming the phase information encoded by the oracle into measurable computational basis states.",
    meas: "Measuring the first n qubits (q[0] to q[2]) deterministically reveals whether the function is constant or balanced. Using the sample function f(x) = 1 for x ∈ {001, 011, 110, 111} and f(x) = 0 for x ∈ {000, 010, 100, 101}, the measurement result indicates that the function is balanced.",
